...
describe('Complete User Workflow E2E Tests', () => {
    ...
    describe('Agent Configuration Workflow', () => {
        it('should configure and interact with an agent', () => {
            ...
            cy.get('a[href*="settings"], button:contains("settings"), a[href*="config"], button:contains("config")')
                .first()
                .then(($elem) => {
                if ($elem.length) {
            ...
            queries.forEach((query, index) => {
                ...
                cy.get('[data-testid*="message"], [class*="message"], [role="article"]').should('have.length.greaterThan', index * 2);
            });
        });
    });
