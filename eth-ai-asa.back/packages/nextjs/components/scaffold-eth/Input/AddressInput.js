...
const AddressInput = ({ value, name, placeholder, onChange, disabled }) => {
    ...
    const { data: ensAddress, isLoading: isEnsAddressLoading, isError: isEnsAddressError, } = (0, wagmi_1.useEnsAddress)({
        name: settledValue,
        chainId: 1,
        query: {
            gcTime: 30000,
    ...
    const { data: ensName, isLoading: isEnsNameLoading, isError: isEnsNameError, } = (0, wagmi_1.useEnsName)({
    ...
    return (<scaffold_eth_1.InputBase name={name} placeholder={placeholder} error={ensAddress === null} value={value} onChange={onChange} disabled={isEnsAddressLoading || isEnsNameLoading || disabled} reFocus={reFocus} prefix={ensName ? (<div className="flex bg-base-300 rounded-l-full items-center">
